      ...
      "import/no-restricted-paths": [
        "error",
        {
          zones: [
            ...
            { target: "./src/shared", from: "./src/entities", message: "[FSD 위반] shared는 entities를 참조할 수 없습니다." },
            { target: "./src/shared", from: "./src/features", message: "[FSD 위반] shared는 features를 참조할 수 없습니다." },
            
            // entities는 비즈니스 로직(features)을 모른 채 독립적이어야 함
            { target: "./src/entities", from: "./src/features", message: "[FSD 위반] entities는 features를 참조할 수 없습니다." },
            
            // features(기능)는 화면 조립 단위인 widgets를 참조할 수 없음
            { target: "./src/features", from: "./src/widgets", message: "[FSD 위반] features는 widgets를 참조할 수 없습니다." },
            
            // widgets는 최종 조립 계층인 app을 참조할 수 없음
            { target: "./src/widgets", from: "./src/app", message: "[FSD 위반] widgets는 app 계층을 참조할 수 없습니다." }
          ]
        }
      ]
    },
