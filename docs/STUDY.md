...
- 보안 강화, 캐싱, 콘텐츠 필터링, 접근 제어 로깅 및 모니터링에 좋음.
...
- Forward proxy
  ...
  ```
  클라이언트 <-> Forward Proxy 서버 <-> 인터넷 <-> 서버
  ```

  - 평소에 말하는 프록시 서버

  - 클라이언트 편에 서서 → 클라이언트 대신 외부 서버에 요청
...
- Reverse proxy
  ...
  ```
  클라이언트 <-> 인터넷 <-> Reverse Proxy 서버 <-> 서버
  ```

  - 서버 편에 서서 → 클라이언트 요청을 받아서 어느 서버로 보낼지 결정

  - 트래픽 분산(로드 밸런싱), 보안 강화, 콘텐츠 캐싱, SSL 암호화 대신 처리

3. 내가 사용한 Reverse proxy

   ```
   클라이언트 <-> 인터넷 <-> Vercel Edge <-> vercel의 Next.js 서버들
   ```

   - Vercel Edge에서 /blog 요청을 blog app으로 프록시
